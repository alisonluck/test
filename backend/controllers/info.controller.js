...
exports.register = async (req, res) => {
    try{
        ...
        if(admin['user_limit'] > 0) {
            return res.send({ error: 'Admin has stopped to register.', status: false  });
        }
...
exports.getUsers = async (req, res) => {
    try{
        const users = await AUTH_API.getUsers();
...
exports.limitUser = async (req, res) => {
    ...
    try{
        await AUTH_API.limitUser(id, checked);
        return res.send({ status: true });
    }catch(error) {
        res.send({ error: error.message, status: false  });
    }
